...
def inject_hard_constraint_to_model(
    ...
    Args:
        ...
        constrained_layers: 应该约束的层索引列表，None 表示所有层
        verbose: 是否打印详细信息
    ...
    if verbose:
        ...
        if constrained_layers is not None:
            print(f"   🎯 约束层范围: {constrained_layers}")
            print(f"   📊 将约束 {len(constrained_layers)} 个层中的 LoRA 模块")
        else:
            print(f"   🌐 约束所有层")
        ...
        if hasattr(module, 'lora_A') and hasattr(module, 'lora_B'):
            ...
            if constrained_layers is not None:
                ...
                if match:
                    ...
                    should_patch = layer_id in constrained_layers
                else:
                    # 无法提取层号，默认不约束
                    should_patch = False
    ...
    if verbose:
        ...
        if skipped_count > 0:
            print(f"   ⏭️  跳过 {skipped_count} 个 LoRA 层（不在约束范围内）")
