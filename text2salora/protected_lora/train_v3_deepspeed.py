...
from lora_svd_init import initialize_lora_weights
...
from protected_lora.layer_utils import parse_layer_ranges
...
class OrthogonalLoRATrainer(Trainer):
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        """🔑 重写training_step,在optimizer.step()后添加重投影"""
        # 调用父类的training_step
        loss = super().training_step(model, inputs, num_items_in_batch)
        
        # 🔑 Bug Fix 4: 在optimizer更新后,重新投影B矩阵到正交补空间
        if self.use_orthogonal and self.constraint is not None:
            real_model = model.module if hasattr(model, 'module') else model

            with torch.no_grad():
                for name, module in model.named_modules():
                    if isinstance(module, LoraLayer) and hasattr(module, 'lora_B'):
                        # 获取该层的投影矩阵
                        if hasattr(module, 'lora_C') and module.lora_C is not None:
                            C = module.lora_C
                            # 重投影: B' = C @ B
                            for adapter_name, B_param in module.lora_B.items():
                                B = B_param.weight
                                out_dim = B.size(0)
                                
                                # 裁剪C到当前层的out_dim
                                C_block = C[:out_dim, :out_dim]
                                with GatheredParameters([B], modifier_rank=0):
                                    new_B = torch.matmul(C_block, B.data)
                                    B.data.copy_(new_B)
...
def train(
    ...
    lora_dropout: float = 0.1,
    target_modules: List[str] = None,
    lora_init_method: str = 'random',  # 新增: LoRA 初始化方法
    svd_niter: int = 30,  # 新增: SVD 迭代次数
    ...
    subspace_rank: int = None,  # 新增: 使用子空间的前 k 个向量（None=使用全部）
    constrained_layers: str = 'all',  # 新增: 指定应用正交约束的层区间
    ...
    if use_hard_constraint:
        ...
        if subspace_dir is None or preference_dimensions is None:
            ...
            # 🔑 修复: 正确计算正交补投影矩阵 C = I - V @ V^T
            # 方案: 合并所有维度的V矩阵,再做QR正交化,最后计算 I - Q @ Q^T
            print(f"   收集所有维度的子空间...")
            V_all = []
            for dim in preference_dimensions:
                V = constraint.manager.get_subspace(dim, layer_id=None)
                V_all.append(V)
                print(f"     {dim}: V shape={V.shape}")
            
            # 合并并正交化
            if len(V_all) == 1:
                V_combined = V_all[0]
            else:
                V_combined = torch.cat(V_all, dim=1)  # [hidden_dim, total_rank]
                print(f"   合并后 V_combined shape: {V_combined.shape}")
                
                # QR分解保证正交性
                Q, _ = torch.linalg.qr(V_combined)
                V_combined = Q
                print(f"   QR正交化后 shape: {V_combined.shape}")
            
            # 🔑 关键: 计算正交补投影矩阵 C_S = I - V @ V^T
            I = torch.eye(V_combined.size(0), device=V_combined.device, dtype=V_combined.dtype)
            C_combined = I - V_combined @ V_combined.T
            print(f"   ✅ 正交补投影矩阵 C shape: {C_combined.shape}")
            
            # 解析层区间
            # 获取模型层数
            num_layers = len([n for n in model.named_modules() if '.layers.' in n[0] and 'self_attn.q_proj.base_layer' in n[0]])
            if num_layers == 0:
                # 如果无法自动获取，使用默认值（Qwen2.5-1.5B 是 28 层）
                num_layers = 28
            
            print(f"\n🔍 检测到模型层数: {num_layers}")
            print(f"   约束层配置: {constrained_layers}")
            
            # 解析层区间
            if constrained_layers.lower() == 'all':
                constrained_layer_list = None  # None 表示所有层
                print(f"   ✅ 将约束所有 {num_layers} 层")
            else:
                constrained_layer_list = parse_layer_ranges(constrained_layers, num_layers)
                print(f"   ✅ 将约束以下层: {constrained_layer_list}")
                print(f"   📊 共 {len(constrained_layer_list)} 个层")
            
            # 注入硬约束到模型
            patched_count = inject_hard_constraint_to_model(
                model=model,
                lora_C=C_combined,
                constrained_layers=constrained_layer_list,
                verbose=True
            )
            ...
            # 🔑 Bug Fix 5: W' 重参数化 - 将预训练权重投影到正交补空间
            print(f"\n🔧 应用 W' 重参数化...")
            with torch.no_grad():
                ...
                for name, module in model.named_modules():
                    if hasattr(module, 'lora_B') and hasattr(module, 'lora_A'):
                        ...
                        if hasattr(module, 'base_layer'):
                            W = module.base_layer.weight  # [out_dim, in_dim]
                            
                            # 计算 BA
                            for adapter_name in module.lora_B.keys():
                                B = module.lora_B[adapter_name].weight  # [out_dim, rank]
                                A = module.lora_A[adapter_name].weight  # [rank, in_dim]
                                BA = B @ A  # [out_dim, in_dim]
                                
                                # W' = W - C @ BA (从W中减去投影到偏好子空间的部分)
                                if hasattr(module, 'lora_C') and module.lora_C is not None:
                                    C = module.lora_C  # [hidden_dim, hidden_dim]
                                    out_dim = W.size(0)
                                    
                                    # 裁剪C到当前层的out_dim
                                    C_block = C[:out_dim, :out_dim]
                                    C_BA = torch.matmul(C_block, BA)
                                    W.data = W.data - C_BA
                                    reparameterized_count += 1
                
                print(f"   ✅ 完成 {reparameterized_count} 个层的 W' 重参数化")
                print(f"   公式: W' = W - C @ (B @ A)")
            
            
            # 4. LoRA 权重初始化（在约束注入之后）
            if lora_init_method != 'random':
                print(f"\n🎲 应用 LoRA 初始化方法: {lora_init_method}")
                try:
                    # 硬约束模式下，constraint 应该从前面加载的约束中获取
                    init_constraint = constraint if constraint is not None else load_subspace_constraint(
                        subspace_dir=subspace_dir,
                        dimensions=preference_dimensions,
                        device=device
                    )
                    
                    initialized_count = initialize_lora_weights(
                        model=model,
                        constraint=init_constraint,
                        rank=lora_rank,
                        method=lora_init_method,
                        niter=svd_niter,
                        verbose=True
                    )
                    print(f"✅ 成功初始化 {initialized_count} 个 LoRA 模块")
                except Exception as e:
    ...
    # 4.5 启用 embedding 层的输入梯度 (gradient checkpointing 需要)
    if use_gradient_checkpointing:
        print(f"\n🔧 启用输入梯度 (gradient checkpointing)")
...
def main():
    ...
    parser.add_argument('--target_modules', type=str, nargs='+', 
                        ...
                        help='LoRA target modules (e.g., q_proj k_proj v_proj o_proj gate_proj up_proj down_proj)')
    parser.add_argument('--lora_init_method', type=str, default='random',
                        choices=['random', 'svd', 'svd_ortho', 'svd_salora'],
                        help='LoRA initialization method: '
                             'random (default PEFT), '
                             'svd (PiSSA), '
                             'svd_ortho (SVD + orthogonal projection, recommended), '
                             'svd_salora (original SaLoRA method)')
    parser.add_argument('--svd_niter', type=int, default=30,
                        help='Number of iterations for SVD decomposition')
    ...
    # 数据
    parser.add_argument('--constrained_layers', type=str, default='all',
                        help='指定应用正交约束的层区间。格式: "all"(所有层), "8-16"(8到16层), "8-16,20-24"(多个区间)')
    ...
    trainer = train(
        ...
        lora_init_method=args.lora_init_method,  # 新增
        svd_niter=args.svd_niter,  # 新增
        ...
        constrained_layers=args.constrained_layers,  # 新增: 层区间约束
        # 训练监控
