...
#================================================================
# 批量测试多个LoRA checkpoints的多位置探针准确度 (带ABC约束)
#================================================================
# 功能: 
...
while [[ $# -gt 0 ]]; do
  ...
  --subspace_dir PATH     子空间目录 (默认: ../../preference_subspace/saved_subspaces)"
            echo "  --help                  显示此帮助信息"
            echo ""
            echo "示例:"
...
echo -e "${CYAN}🧪 批量测试多位置探针准确度 (带ABC约束)${NC}" | tee -a "$LOG_FILE"
echo -e "${CYAN}================================================================================${NC}" | tee -a "$LOG_FILE"
...
echo -e "${BLUE}📁 子空间目录:${NC} $SUBSPACE_DIR" | tee -a "$LOG_FILE"
...
for i in "${!FILTERED_CHECKPOINTS[@]}"; do
    ...
    # 构建测试命令
    CMD="python test_multi_position_probe_accuracy_with_abc.py \
