...
def patch_lora_linear_forward(lora_module, lora_C: Optional[torch.Tensor] = None):
    ...
    修改: output = base(x) + (lora_B(lora_A(x)) * scaling) @ lora_C.T
        ...
        for active_adapter in self.active_adapters:
            ...
            if not self.use_dora[active_adapter]:
                ...
                if hasattr(self, 'lora_C') and self.lora_C is not None:
                    # lora_output shape: (batch*seq_len, out_dim)
                    # lora_C shape: (hidden_dim, hidden_dim)
                    # 
                    # 🔑 关键: 裁剪 C 到当前层的 out_dim
                    # 因为不同层的 out_dim 不同 (q_proj, k_proj, v_proj 等)
                    out_dim = lora_output.size(-1)
                    lora_C = self.lora_C.detach().to(lora_output.device, dtype=lora_output.dtype)
                    
                    # 裁剪投影矩阵到 [out_dim, out_dim]
                    C_block = lora_C[:out_dim, :out_dim]
                    
                    # 投影到正交补空间: output' = output @ C
                    lora_output = torch.matmul(lora_output, C_block)
...
def inject_hard_constraint_to_model(
    ...
    if verbose:
        ...
        print(f"   🔒 约束: LoRA 输出 @ C^T (硬约束，数学上严格保证)")
