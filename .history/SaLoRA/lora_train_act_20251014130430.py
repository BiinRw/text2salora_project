...
def main():
    ...
    # 修改: 使用更清晰的目录命名方式
    module_suffix = 'more' if args.more_module else 'base'
    output_dir = f'out/{args.rs}_{args.ds}_{module_suffix}_qwen_output'
    args.name = f'{output_dir}/qwen_{args.name}_rank{args.rank_s}'
    
    # 修改: 使用 os.makedirs 并添加 exist_ok=True 参数
    # 这样可以创建多级目录,且如果目录已存在不会报错
    try:
    ...
    model = AutoModelForCausalLM.from_pretrained('Qwen/Qwen2.5-1.5B-Instruct', device_map="cuda:0")
    if args.more_module:
    ...
    save_path = f"{args.name}_{rank}_{args.lr}_{args.wd}_lora_ABC.pt"
    print(f"✓ 保存 LoRA 初始化权重到: {save_path}")
    torch.save(weight_list, save_path)
    del weight_list
    torch.cuda.empty_cache()
    # ===== 加载训练数据集 =====
    print("" + "=" * 70)
    print("📦 开始加载 alpaca-cleaned 数据集...")
    print("=" * 70)
    ...
    try:
        print("🔄 正在从 Hugging Face Hub 加载 yahma/alpaca-cleaned...")
        print("   (首次加载会下载数据集,大约 44MB,请耐心等待)")
        ...
        print(f"❌ 数据集加载失败: {type(e).__name__}")
        print(f"   错误信息: {str(e)}")
        print("" + "=" * 70)
        print("🔧 解决方案:")
        print("1. 检查网络连接")
        print("2. 清理缓存: rm -rf ~/.cache/huggingface/datasets/yahma___alpaca-cleaned")
        print("3. 手动下载: wget https://huggingface.co/datasets/yahma/alpaca-cleaned/resolve/main/alpaca_data_cleaned.json")
        print("=" * 70)
        raise
    
    # 构建训练提示词 (使用 Qwen2.5 格式)
    print(f"📝 开始构建训练提示词...")
    count = 0
    
    for line in tqdm(dataset, desc="处理数据"):
        count += 1
        instruction = line.get('instruction', '').strip()
        input_text = line.get('input', '').strip()
        output_text = line.get('output', '').strip()
        
        # 使用 Qwen2.5 对话格式
        if input_text:
            prompt = (
                f"<|im_start|>user{instruction} {input_text}<|im_end|>"
                f"<|im_start|>assistant{output_text}<|im_end|>"
            )
        else:
            prompt = (
                f"<|im_start|>user{instruction}<|im_end|>"
                f"<|im_start|>assistant{output_text}<|im_end|>"
            )
        
        prompts.append({'text': prompt})
    
    print(f"✅ 提示词构建完成: {count} 条训练样本")
    print("=" * 70 + "")

    df = Dataset.from_list(prompts)
    
    # 构建训练输出目录
    training_output_dir = f"{args.name}_{rank}_{args.lr}_{args.wd}"
