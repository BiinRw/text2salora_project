...
DEFAULT_ABC_PATH = "/home/wangbinrui/research_projects/text-to-salora/SaLoRA/out/safety-lora_wo_g_r16_a32-ep1-svd_rank16-salora-lr_5e-5/constraint_on_layer_all_ABC.pt"
NUM_LAYERS = 28
...
def apply_abc_constraints(model, weight_list: Dict, disable_layers: List[int] = None):
    """应用ABC约束到模型
    
    Args:
        model: vLLM模型
        weight_list: 从ABC.pt加载的权重字典
        disable_layers: 要禁用约束的层列表（恢复原始权重）
    """
    ...
    for name, module in model.llm_engine.model_executor.driver_worker.model_runner.model._modules['model'].named_modules():
        if 'self_attn.qkv_proj.base_layer' in name:
            layer_id = current_num // 2
            
            # 检查是否要禁用这层的约束
            if layer_id in disable_layers:
                print(f"  ⏭️  跳过第{layer_id}层约束（已禁用）")
                current_num += 2
                continue
            
            # 应用ABC约束
            q_key = 'q_proj_' + str(layer_id) + 'weight'
            v_key = 'v_proj_' + str(layer_id) + 'weight'
            
            if q_key in weight_list and v_key in weight_list:
                tmp_data = torch.cat([
                    weight_list[q_key].to(module.weight.dtype).to(module.weight.device),
                    module.weight.data[weight_list[q_key].size(0):-weight_list[v_key].size(0), :],
                    weight_list[v_key].to(module.weight.dtype).to(module.weight.device)
                ], dim=0)
...
def generate_and_evaluate_config_with_abc(config_name: str, disable_layers: List[int], 
    ...
    # ===== 第1步：加载ABC约束 =====
    print(f"\n📥 加载ABC约束: {abc_path}")
    weight_list = torch.load(abc_path, map_location=torch.device('cpu'))
    print(f"✅ 约束加载完成，共{len(weight_list)}个权重")
    
    # ===== 第2步：使用vLLM加载模型并应用ABC约束 =====
    print(f"\n�� 使用vLLM加载模型...")
    model = LLM(
        model=DEFAULT_MODEL_PATH,
        tokenizer=DEFAULT_MODEL_PATH,
        dtype='float16',
        max_model_len=256,
        gpu_memory_utilization=0.75,
    )
    ...
    print(f"🔗 应用ABC约束...")
    apply_abc_constraints(model, weight_list, disable_layers)
    print(f"✅ 约束应用完成")
    
    # ===== 第3步：生成回复 =====
    print(f"\n🚀 生成回复...")
    formatted_prompts = [f"[INST]{p}[/INST]" for p in test_prompts]
    sampling_params = SamplingParams(temperature=0.7, max_tokens=100, top_p=0.9)
