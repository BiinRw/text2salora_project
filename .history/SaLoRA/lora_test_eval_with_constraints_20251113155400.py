...
weight_path = "./out/safety-lora_wo_g_r16_a32-ep1-svd_rank16-salora-lr_5e-5/constraint_on_layer17_27_ABC.pt" ###Change to your own ABC
...
# ========== 关键修改:指定 GPU 0 加载主模型 ==========
os.environ['CUDA_VISIBLE_DEVICES'] = '2'  # 只让 vLLM 看到 GPU 0
model = LLM(model=model_path,
            tokenizer=model_path,
            enable_lora=True, 
            max_lora_rank=64,
            gpu_memory_utilization=0.8,  # GPU 0 可以用更多显存
            max_model_len=2048,
            dtype='half')
tokenizer=AutoTokenizer.from_pretrained(model_path)
lora_path = '../text2salora/protected_lora/output/safety-lora_wo_g_r16_a32-ep1-svd_rank16-salora_17_27-lr_5e-5' ###Lora adapter path
...
with open('./lowrank_prune/data/harm_test.csv','r') as f:
    ...
    for suffix in template:
        ...
        outputs = model.generate(prompts, sampling_params)  # 不需要 lora_request,已用 ABC.pt weight 替换
        if len(results) == 0:
...
# ========== 关键修改:切换到 GPU 1 加载 LlamaGuard ==========
print("\n🔄 切换到 GPU 1 加载 LlamaGuard...")
os.environ['CUDA_VISIBLE_DEVICES'] = '3'  # 切换到 GPU 1

cls_path = "meta-llama/Llama-Guard-3-8B"
cls_model = LLM(model=cls_path, 
                tokenizer=cls_path,
                dtype='half',
                max_model_len=4096,  # LlamaGuard 需要较长上下文
...
print(f"\n✅ LlamaGuard 评估完成!")
print(f"   有害内容比例: {harm_num/len(preds):.2%} ({harm_num}/{len(preds)})")
